...
def test_portable_ci_targets_are_explicit_and_retain_all_non_runtime_gates() -> None:
    ...
    assert portable_private_recipe.group(1).strip() == (
        "$(MAKE) ci-common-private ci-portable-topology check-portable-defect-closure check-p0-baseline check-p0-maintainability check-test-governance-topology "
    ...
    for gate in ("audit-portable", "check-d0-closure", "check-contracts", "check-secrets", "check-python-static", "typecheck-dashboard", "lint-dashboard", "build-dashboard", "audit-python-source", "audit-dependencies"):
        assert gate in common_private_recipe.group(1)
    for duplicate in ("test-backend", "test-dashboard"):
        assert duplicate not in common_private_recipe.group(1)
    assert portable_private_recipe.group(1).split().count("check-test-governance-topology") == 1
