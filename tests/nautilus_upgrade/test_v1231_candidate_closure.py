...
def test_candidate_bwrap_handoff_binds_only_the_verified_physical_stage_fd(
    ...
    assert "/home/thenam176/.cache/trading-agent/nautilus" not in command
    ...
    try:
        initial, _effective = builder._candidate_environment(
            builder._candidate_json(ENGINE_POLICY), logical_stage, source_fd
        )
    finally:
        os.close(source_fd)
    entry_environment = {
...
def test_candidate_package_preamble_accepts_poetry_vendored_import_path(
    ...
    poetry = next(
        record
        for record in inputs["build_wheels"]
        if record["package"] == "poetry-core"
    )
    assert poetry == {
        "active_dependencies": [],
        ...
        "size": 340949,
        "source_classification": "UPSTREAM_BUILD_SYSTEM_EXACT_PIN",
        "tags": ["py3-none-any"],
        "url": "https://files.pythonhosted.org/packages/01/10/c9f7ca739ab0956f8dc8737e6318b9812745589ed1e3f6fdfb6a5d64f58a/poetry_core-2.3.1-py3-none-any.whl",
        "version": "2.3.1",
    }
    wheel_root = Path(inputs["python"]["explicit_build_path_admission"]["root"])
    wheel = wheel_root / poetry["filename"]
    assert hashlib.sha256(wheel.read_bytes()).hexdigest() == poetry["sha256"]
    logical_stage = tmp_path / "stage-0123456789abcdef"
    ...
    result = subprocess.run(
        ["/usr/bin/python3.12", "-I", "-S", "-c", preamble],
        cwd=tmp_path,
        capture_output=True,
        text=True,
        timeout=30,
...
def test_candidate_binds_exact_upstream_build_and_pyproject_symbols(tmp_path: Path) -> None:
    engine = builder._candidate_json(ENGINE_POLICY)
    inputs = builder._candidate_json(builder._CANDIDATE_TOOLCHAIN_INPUTS)
    source = tmp_path / "source"
    source.mkdir()
    record = inputs["source"]["artifact"]
    archive_path = (
        builder._candidate_roots(engine)["candidate_input_root"]
        / "source-inputs"
        / record["filename"]
    )
    with tarfile.open(archive_path, "r:gz") as archive:
        for name in ("build.py", "pyproject.toml"):
            member = archive.getmember(f"{record['top_level_root']}/{name}")
            extracted = archive.extractfile(member)
            assert extracted is not None
            (source / name).write_bytes(extracted.read())
...
def test_real_candidate_bwrap_has_only_u03_admitted_runtime_surface(tmp_path: Path) -> None:
    physical_stage = tmp_path / "physical-stage"
    source = physical_stage / "source"
    source.mkdir(parents=True)
    source.chmod(0o700)
...
def test_real_candidate_bwrap_clang_default_host_link_uses_sealed_ld_alias(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    physical_stage = tmp_path / "physical-stage"
    source = physical_stage / "source"
...
def test_candidate_attestor_routes_engine_identity_only_to_the_engine_wheel(
    ...
    runtime_wheel.write_bytes(
        (roots["candidate_input_root"] / "wheels" / click["filename"]).read_bytes()
    )
    ...
    loader_raw = Path("/usr/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2").read_bytes()
...
SAFE_BWRAP = (
    ...
    '/usr/bin/python3',
    '-c',
    'from pathlib import Path; p=Path("/tmp/observed"); p.mkdir(); '
    'f=p/"a"; f.write_bytes(b"x"); f.chmod(0o400); f.rename(p/"b"); '
    '(p/"b").unlink(); p.rmdir()',
)
...
def reject_host_mutation(event, args):
    ...
    if event == 'open':
        ...
        mutated = tuple(args[1]) != SAFE_BWRAP
    if mutated:
...
def qualify(_root):
    subprocess.run(SAFE_BWRAP, env={{}}, capture_output=True, check=True, timeout=30)
