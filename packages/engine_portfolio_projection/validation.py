...
_P1_MAXIMUM_QUANTITY = Decimal("1000000")
_P1_MAXIMUM_NOTIONAL = Decimal("100000000")
...
def _instrument_from_catalog(
    catalog: P1InstrumentCatalogV1, *, observed_at: datetime
) -> InstrumentDefinition:
    if (
        -int(catalog.tick_size.as_tuple().exponent) != catalog.price_precision
        ...
        raise ProjectionError("catalog precision does not match exact P1 catalog")
    try:
        return InstrumentDefinition(
            ...
            maximum_quantity=OrderQuantity(
                _P1_MAXIMUM_QUANTITY, catalog.size_precision
            ),
            minimum_notional=Money(catalog.min_notional, Currency.USDT),
            maximum_notional=Money(_P1_MAXIMUM_NOTIONAL, Currency.USDT),
        ...
        raise ProjectionError("catalog cannot build exact P1 instrument") from exc
...
def canonical_authority(value: object) -> ProjectionAuthority:
    ...
    expected_instrument = _instrument_from_catalog(
        catalog, observed_at=opening.effective_at
    )
