...
class CustodianReleaseRepository:
    @classmethod
    def from_systemd_credentials(cls,values: Mapping[str,str]) -> 'CustodianReleaseRepository':
    ...
    def _execute(self,query: LiteralString,request: ReleaseRequest) -> datetime:
        ...
        with psycopg.Connection[dict[str,object]].connect(
            ...
            if connection.execute(CATALOG_SQL).fetchone()!={'catalog_sha256':CATALOG_SHA256}:
