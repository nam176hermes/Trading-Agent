...
def _effective_mode(
    ...
    if requested is _SafetyMode.UNKNOWN:
        return _SafetyMode.UNKNOWN
    if requested is not _SafetyMode.LIVE:
        return requested
    if (
        execution_enabled is None
        or trading_approved is None
        or kill_switch is CanonicalKillSwitchState.UNKNOWN
    ):
        return _SafetyMode.UNKNOWN
    if (
        execution_enabled
        and trading_approved
        and kill_switch is CanonicalKillSwitchState.INACTIVE
    ):
        return _SafetyMode.LIVE
