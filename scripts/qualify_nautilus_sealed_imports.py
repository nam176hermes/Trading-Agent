...
def _read_file(
    ...
    *,
    ...
    try:
        ...
        return b"".join(chunks), (opened.st_dev, opened.st_ino, opened.st_size, mode)
...
def _private_parent(path: Path) -> tuple[int, int]:
    try:
        observed = path.lstat()
    except OSError as exc:
...
def _repository_launcher_bytes(
    ...
    for value in policy["launcher_inventory"]:
        ...
        raw, _identity = _read_file(
            source, label="repository launcher", modes=None
        )
...
def _sandbox_identity(sandbox: Path) -> bytes:
    common = {
        "cwd": Path("/"),
        "env": {},
    ...
    try:
        version = subprocess.run((str(sandbox), "--version"), **common)
        help_result = subprocess.run((str(sandbox), "--help"), **common)
...
def _publish_receipt(path: Path, raw: bytes, parent_identity: tuple[int, int]) -> None:
    current = path.parent.lstat()
    if (
        (current.st_dev, current.st_ino) != parent_identity
    ...
    try:
        temporary_fd, name = tempfile.mkstemp(prefix=f".{path.name}.", dir=path.parent)
...
def qualify_sealed_imports(
    *,
    ...
    parent_identity = _private_parent(receipt_path.parent)

    policy_raw, _ = _read_file(policy_path, label="runtime closure policy", modes={0o400})
    sandbox_raw, _ = _read_file(sandbox_executable, label="Bubblewrap executable", modes={0o500})
    probe_raw, _ = _read_file(_PROBE, label="sealed import probe", modes=None)
    try:
        policy = _closure._load_policy(policy_path)
        _base_manifest, records = _closure._validate_base_runtime(base_runtime, policy)
        _artifact_manifest, selected_wheel = _closure._validate_artifact(artifact_directory, policy)
    except (OSError, ValueError) as exc:
        raise SealedImportQualificationError(str(exc)) from exc
    base_manifest_raw, _ = _read_file(
        base_runtime / "closure-manifest.json", label="base runtime manifest", modes={0o400}
    )
    artifact_manifest_raw, _ = _read_file(
        artifact_directory / "artifact-manifest.json", label="artifact manifest", modes={0o400}
    )
    selected_wheel_raw, _ = _read_file(selected_wheel, label="selected engine wheel", modes={0o400})
    launchers, launcher_records = _repository_launcher_bytes(policy)
    launcher_by_target = {record["target"]: record for record in launcher_records}
    entry_target = str(policy["argv_prefix"][3])
    if entry_target not in launchers or str(_STRATEGY_TARGET) not in launchers:
        raise SealedImportQualificationError("policy launchers do not cover qualification targets")
    ...
    for record in records:
        ...
        if target == _PYTHON_TARGET:
            if python_raw is not None or mode != 0o500:
                raise SealedImportQualificationError("selected CPython inventory is invalid")
            python_raw = raw
    ...
    for target_text, (raw, mode, _source) in launchers.items():
        mounts.setdefault(PurePosixPath(target_text), (raw, mode))
    if python_raw is None or not wheel_inventory:
        raise SealedImportQualificationError("runtime lacks CPython or sealed wheels")
    ...
    _sandbox_identity(sandbox_executable)

    descriptors: list[int] = []
    try:
        sandbox_fd = _sealed_memfd("qualification-bwrap", sandbox_raw, mode=0o500)
        descriptors.append(sandbox_fd)
        mounted_fds: list[tuple[PurePosixPath, int, int]] = []
        for target, (raw, mode) in sorted(mounts.items(), key=lambda item: item[0].as_posix()):
            descriptor = _sealed_memfd(f"qualification-{target.name}", raw, mode=mode)
        ...
        try:
            ...
            raise SealedImportQualificationError("sealed import probe execution failed") from exc
    finally:
        for descriptor in descriptors:
            os.close(descriptor)

    probe_document = _parse_probe_result(result.stdout, result.stderr, result.returncode)
    ...
    if (
        ...
        or any(record["source_wheel_sha256"] not in wheel_digests for record in probe_document["modules"])
    ):
        raise SealedImportQualificationError("probe result is not bound to the mounted inventory")

    try:
        fresh_policy = _closure._load_policy(policy_path)
        _closure._validate_base_runtime(base_runtime, fresh_policy)
        _closure._validate_artifact(artifact_directory, fresh_policy)
        fresh_sandbox, _ = _read_file(sandbox_executable, label="Bubblewrap executable", modes={0o500})
        fresh_probe, _ = _read_file(_PROBE, label="sealed import probe", modes=None)
        fresh_launchers, _ = _repository_launcher_bytes(fresh_policy)
        if (
            fresh_policy != policy
            or fresh_sandbox != sandbox_raw
            or fresh_probe != probe_raw
            or {target: raw for target, (raw, _mode, _path) in fresh_launchers.items()}
            != {target: raw for target, (raw, _mode, _path) in launchers.items()}
        ):
            raise ValueError("named input bytes changed")
    except (OSError, ValueError) as exc:
        raise SealedImportQualificationError("named qualification input became stale") from exc
    ...
    receipt: dict[str, object] = {
        ...
        "native_entry_guard_policy_sha256": _sha256(_canonical(policy["native_entry_guard"])),
        "launcher_inventory_sha256": _sha256(_canonical(policy["launcher_inventory"])),
        "entry_launcher_sha256": _sha256(launchers[entry_target][0]),
        "probe_sha256": _sha256(probe_raw),
        "strategy_source_sha256": launcher_by_target[str(_STRATEGY_TARGET)]["sha256"],
        "minimal_manifest_sha256": _sha256(minimal_manifest),
        "wheel_inventory_sha256": _sha256(_canonical(sorted(wheel_inventory, key=lambda record: str(record["filename"])))),
    ...
    _publish_receipt(receipt_path, _canonical(receipt) + b"\n", parent_identity)
    return receipt_path
