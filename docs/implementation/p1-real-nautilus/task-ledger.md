...
| P1-U04 | P1-U03 | X4_PREFLIGHT_PASS_REVIEW_PENDING | Obtain fresh spec and security/replay approval of the exact X4 HEAD/tree and hash-bound receipt before Build A. |
...
implemented through `ddbecbbdbb6f2f7ffd2389fc33b8e665faeddd7c`. Fresh
implementation reviews report Critical 0, Important 0, and one non-blocking
fail-closed Minor. Read-only X4 replay passed U02 provenance, U03 toolchain,
real Bubblewrap host authority, exact schema-6 rollback, and root
absence/disjointness checks. The current outcome is
`P1_U04_X4_PREFLIGHT_PASS_REVIEW_PENDING`. Native Build A/B, X5, activation,
and promotion remain unauthorized until a new receipt bound to the final
HEAD/tree passes fresh X4 spec and security/replay reviews.
