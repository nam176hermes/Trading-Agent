...
- [ ] **Step 5: Add a reproducible offline runtime-closure materializer.**
  ...
  - The policy binds exact base runtime manifest/artifact manifest digests, expected source launcher path, profile manifest schema, file inventory, modes, and source commit.  The materializer verifies all inputs before copying, produces only 0500 roots/0400 or 0500 regular sealed files, writes a canonical closure manifest, and attests the sealed private staging tree with the root attestor **before** it becomes visible at the final destination.  Publish only through Linux `renameat2(RENAME_NOREPLACE)` (or fail closed if unavailable); `os.replace`/replace-on-exists semantics are forbidden.  Set the published state immediately after a successful rename and defer parent-FD cleanup until after final destination identity checks and re-attestation; a cleanup-close error must never route a published generation through failed-publication cleanup or skip final verification.
  ...
  - Tests first: pre-existing destination, a destination created after preflight but before publish (including an empty directory/inode competitor), unsupported no-clobber syscall, a parent-FD close error after successful rename, base/artifact/launcher digest drift, unlisted file, profile mismatch, unsafe file mode, failed pre-publish attestation, and atomic publish failure all leave no unattested selected generation and fail closed.

- [ ] **Step 6: Materialize and verify the selected simulation closure.**
  - After source tests pass, materialize an external generation named `runtime-closure-v8-simulation` below the private Nautilus cache using only the current sealed inputs and the selected 01D artifact.  Do not commit its files.  Retain `runtime-closure-v4-simulation` through `runtime-closure-v7-simulation` as rejected forensic candidates: do not select, overwrite, or delete them.
